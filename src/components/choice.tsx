...
const PROMO_ACCESS_KEY =
  "certipm_promo_access";
...
const quizzes: QuizChoice[] = [
...

  {
    ...
    title:
      "New PMP Practice Quiz - Business Environment",
    description:
...
const Choose: React.FC = () => {
  ...
  */
  ...
  const getWhatsAppLink = (
    ...
    const message =
      `Hello CertiPM, I would like to request access to the ${quiz.title}. Please send me more information about availability, access, and next steps.`;
  ...
  */
  ...
  |
  | Previously, the code was validated but the access
  | was NEVER stored in sessionStorage.
  |
  | Pmpeople.tsx then checked sessionStorage and found
  | nothing, resulting in:
  |
  | "Promo access required"
  |
  | Now we save the access BEFORE opening the modal.
  |
  */
                      ...
                      >
                        ...
                        {quiz.purchaseFullQuiz ? (
                          <button
                            ...
                            className="mt-8 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-red-700 px-6 py-3.5 text-sm font-semibold text-white transition hover:bg-red-800 sm:text-base"
                          >
                            Purchase Full Quiz
                            <ArrowRight className="h-4 w-4" />
                          </button>
